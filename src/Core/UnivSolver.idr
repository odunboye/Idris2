...
leqUnivLevel (USucc _) UZero      = Just False   -- n+1 > 0  (must precede wildcard _ UZero)
leqUnivLevel _         UZero      = Nothing      -- n ≤ 0 only if n = 0 (need to know n)
leqUnivLevel UZero     (USucc _)  = Just True    -- 0 < n+1  (already covered by first case, kept for clarity)
...
applyAssign assign (UVar n)     =
  case lookup n assign of
    ...
    Just 0  => UVar n      -- unconstrained (minimum level 0): keep polymorphic
    Just k  => natToLevel k
...
mutual
  ...
  applyAssignToTerm assign (TFix fc c b)
      = TFix fc (applyAssignToTerm assign c) (applyAssignToTerm assign b)
  applyAssignToTerm assign (TLater fc c t)
      = TLater fc (applyAssignToTerm assign c) (applyAssignToTerm assign t)
  applyAssignToTerm assign (TNext fc c v)
      = TNext fc (applyAssignToTerm assign c) (applyAssignToTerm assign v)
  applyAssignToTerm assign (TTickAbs fc v b)
      = TTickAbs fc (applyAssignToTerm assign v) (applyAssignToTerm assign b)
  applyAssignToTerm assign (TTickApp fc fn a)
      = TTickApp fc (applyAssignToTerm assign fn) (applyAssignToTerm assign a)
