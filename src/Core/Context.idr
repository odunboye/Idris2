...
HasNames (Term vars) where
  ...
  full gam (TFix fc c b)
      = pure (TFix fc !(full gam c) !(full gam b))
  full gam (TLater fc c t)
      = pure (TLater fc !(full gam c) !(full gam t))
  full gam (TNext fc c v)
      = pure (TNext fc !(full gam c) !(full gam v))
  full gam (TTickAbs fc v b)
      = pure (TTickAbs fc !(full gam v) !(full gam b))
  full gam (TTickApp fc f a)
      = pure (TTickApp fc !(full gam f) !(full gam a))
  ...
  resolved gam (TFix fc c b)
      = pure (TFix fc !(resolved gam c) !(resolved gam b))
  resolved gam (TLater fc c t)
      = pure (TLater fc !(resolved gam c) !(resolved gam t))
  resolved gam (TNext fc c v)
      = pure (TNext fc !(resolved gam c) !(resolved gam v))
  resolved gam (TTickAbs fc v b)
      = pure (TTickAbs fc !(resolved gam v) !(resolved gam b))
  resolved gam (TTickApp fc f a)
      = pure (TTickApp fc !(resolved gam f) !(resolved gam a))
...
HasNames Error where
  ...
  full gam (InArg fc fn x err) = InArg fc <$> maybe (pure Nothing) (\n => Just <$> full gam n) fn <*> full gam x <*> full gam err
  ...
  resolved gam (InArg fc fn x err) = InArg fc <$> maybe (pure Nothing) (\n => Just <$> resolved gam n) fn <*> resolved gam x <*> resolved gam err
