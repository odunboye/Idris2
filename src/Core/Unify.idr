...
mutual
  ...
  -- A local variable against a data constructor: try record η-equality first.
  -- This enables proofs like `swap (swap p) = p` by Refl, where the LHS
  -- normalises to `MkR (fst p) (snd p)` which equals `p` via tryRecordEta.
