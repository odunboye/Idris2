...
expandAmbigName mode nest env orig args (IVar fc x) exp
  ...
  where
    -- When %prefix_record_projections is on, looking up a field name finds both
    -- the prefix accessor (NS ns (UN (Basic "f"))) and the field projection
    -- (NS ns (UN (Field "f"))). Since the prefix form is just an alias for the
    -- field form, drop it if both are present to avoid a spurious ambiguity.
