...
mutual
  ...
  unelabTy' umode nest env (TTickAbs fc _ _) = pure (Implicit fc True, gErased fc)
  unelabTy' umode nest env (TTickApp fc _ _) = pure (Implicit fc True, gErased fc)
  ...
  unelabBinder umode nest fc env x (Pi _ rig p ty) sctm sc scty
      = do (ty', _) <- unelabTy umode nest env ty
           ...
           pure (IPi fc rig p' nm ty' sc, gType fc (UVar (MN "top" 0)))
    where
  ...
  unelabBinder umode nest fc env x (PVTy _ rig ty) sctm sc scty
      = do (ty', _) <- unelabTy umode nest env ty
           pure (sc, gType fc (UVar (MN "top" 0)))
