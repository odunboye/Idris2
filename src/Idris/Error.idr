...
Eq Error where
  ...
  IrrelevantUsed fc n == IrrelevantUsed fc' n' = n == n'
  ...
  UniverseInconsistency fc1 l1 r1 d1 == UniverseInconsistency fc2 l2 r2 d2
      = fc1 == fc2 && l1 == l2 && r1 == r2 && d1 == d2
...
perrorRaw (LinearMisuse fc n exp ctx)
  ...
  where
    ...
    prettyRel = elimSemi "irrelevant"
                         "relevant"
                         (const "non-linear")
perrorRaw (IrrelevantUsed fc n) = pure $
    errorDesc (reflow "Irrelevant variable" <++> code (pretty0 n) <++> reflow "used in a relevant position.")
    <+> line <+> reflow "Variables bound by .(x : A) -> B may not appear in computationally relevant positions."
...
perrorRaw (UniverseInconsistency fc l r details)
    = pure $ reflow "Universe level error:" <++> pretty0 (show l)
        <++> reflow "is not <=" <++> pretty0 (show r)
        <+> (if details == "" then emptyDoc
             else line <+> indent 2 (pretty0 details))
        <+> line <+> !(ploc fc)
