...
prettyImport (MkImport loc reexport path nameAs spec)
  ...
  where
    ...
    prettySpec (Hiding ns) = space <+> keyword "hiding"
                             <++> parens (concatWith (surround (comma <+> space)) (map pretty0 ns))
    prettySpec (Explicit ns) = space <+> parens (concatWith (surround (comma <+> space)) (map prettyName ns))
